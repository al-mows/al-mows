...
export default function SectionHeading({
  ...
  const introColor = tone === "dark" ? "text-brand-brown" : "text-brand-cream/85";
  ...
  return (
    <div className={`flex flex-col ${alignment}`}>
      ...
      <h2
        className={`mt-2 max-w-3xl font-heading text-3xl font-bold uppercase leading-tight sm:text-4xl lg:text-5xl ${titleColor}`}
      >
        {title}
      </h2>
      {intro && (
        <p className={`mt-4 max-w-2xl text-base leading-relaxed sm:text-lg ${introColor}`}>
          {intro}
        </p>
      )}
    </div>
